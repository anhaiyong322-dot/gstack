...
### 9. Keeping This Fork Current with Upstream

This fork now includes an overlay maintenance layer so you can keep tracking upstream `garrytan/gstack` without throwing away the Codex and Windows customizations.
...
Use [Upstream Sync Guide](UPSTREAM-SYNC.md) when you want to:

- check whether upstream `main` moved ahead
- create a fresh trial worktree from the latest upstream commit
- re-apply the overlay
- refresh `overlay/files/` and `overlay/patches/` after validating the new baseline
