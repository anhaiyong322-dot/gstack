...
这份 fork 现在已经带了一层 overlay 维护结构，目标不是长期和上游分叉，而是尽量保持：

```text
上游最新版 gstack
        +
这份 fork 的 Codex / Windows 定制层
        =
你持续可维护的版本
```

当你想跟进上游 `garrytan/gstack` 的最新提交时，直接看 [Upstream Sync Guide](UPSTREAM-SYNC.md)。
...
- 基于最新上游创建试验 worktree
- 重新套用 overlay
- 在验证完成后刷新 `overlay/files/` 和 `overlay/patches/`
